...
```rust
// Step 1: Specify and build your model

let model = ModelBuilder::new(dataset_spec)
    // Some configuration is tied to the model 
    .batch_size(128)
    .loss(Losses::BCE)
    .epochs(20)
...
// Step 2: Enrich the features of your data (eg. the "columns") with metadata using Tags
...
// The pipeline can result in new features, so don't forget to attach the
// new dataset specification generated by the pipeline to the model
let model = model.with_new_dataset(dataset_spec);
