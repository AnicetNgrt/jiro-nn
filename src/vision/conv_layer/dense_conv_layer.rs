use crate::vision::{Image, conv_optimizer::ConvOptimizers, conv_initializers::ConvInitializers, image::ImageTrait};
...
impl DenseConvLayer {
    ...
    pub fn forward(&mut self, input: Image) -> Image {
        let res = input.cross_correlate(&self.kernels).component_add(&self.biases);
    ...
    pub fn backward(&mut self, epoch: usize, output_gradient: Image) -> Image {
        let input_grad = 
            output_gradient.convolve_full(&self.kernels);

        let kern_grad =
            self.input.as_ref().unwrap().cross_correlate(&output_gradient);
        ...
        self.kernels = self.kernels_optimizer.update_parameters(epoch, &self.kernels, &kern_grad);
        self.biases = self.biases_optimizer.update_parameters(epoch, &self.biases, &biases_grad);
        input_grad
    }
}